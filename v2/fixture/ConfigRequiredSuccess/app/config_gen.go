...
func LoadConfig() (cfg *Config, errs []error) {
	...
	if ok {
		...
		if cfg.FldMapReq == nil {
			errs = append(errs, errors.New("env FLD_MAP_REQ required"))
		}
	} else {
		errs = append(errs, errors.New("env FLD_MAP_REQ required"))
	}
	fldMapIntReqTmp, ok := os.LookupEnv("FLD_MAP_INT_REQ")
	if ok {
		partsfldmapintreq := strings.Split(fldMapIntReqTmp, ",")
		cfg.FldMapIntReq = make(map[string]int, len(partsfldmapintreq))
		for _, s := range partsfldmapintreq {
			kv := strings.Split(s, "=")
			if len(kv) == 2 {
				tmpInt, err := strconv.Atoi(kv[1])
				if err != nil {
					errs = append(errs, fmt.Errorf("convert FLD_MAP_INT_REQ error: %w", err))
				}
				cfg.FldMapIntReq[kv[0]] = int(tmpInt)
			}
		}
		if cfg.FldMapIntReq == nil {
			errs = append(errs, errors.New("env FLD_MAP_INT_REQ required"))
		}
	} else {
		errs = append(errs, errors.New("env FLD_MAP_INT_REQ required"))
	}
...
FLD_MAP_INT_REQ=` + fmt.Sprintf("%v", cfg.FldMapIntReq) + ` ; Test map[string]int required description
`
	return out
}
