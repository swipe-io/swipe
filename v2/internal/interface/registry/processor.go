...
func (r *registryProcessor) NewProcessor(o *option.ResultOption, externalOptions []*option.ResultOption, data *astloader.Data) (up.Processor, error) {
	...
	case "Service":
		sg, err := ig.NewServiceGateway(o.Pkg, data.Module.Dir, o.Option, data.GraphTypes, data.CommentFuncs, data.CommentFields, data.Enums, data.WorkDir, externalOptions)
		if err != nil {
			return nil, err
		}
