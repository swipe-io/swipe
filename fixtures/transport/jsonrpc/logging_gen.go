...
func (s *loggingMiddlewareSwipe) Get(ctx context.Context, id int, name string, fname string, price float32, n int, b int, c int) (result user.User, err error) {
	defer func(now time.Time) {
		s.logger.Log("method", "Get", "took", time.Since(now), "id", id, "name", name, "fname", fname, "price", price, "n", n, "b", b, "c", c, "result", result, "err", err)
	}(time.Now())
...
func (s *loggingMiddlewareSwipe) TestMethod(data map[string]interface{}, ss interface{}) (result map[string]map[int][]string, err error) {
	defer func(now time.Time) {
