...
func ServiceInterfaceTestMethod2ClientOptions(opt ...jsonrpc.ClientOption) (_ ServiceInterfaceClientOption) {
...
func NewClientJSONRPCServiceInterface(tgt string, opts ...ServiceInterfaceClientOption) (service.Interface, error) {
	...
	c.testMethod2ClientOption = append(
		...
		jsonrpc.ClientRequestEncoder(func(_ context.Context, obj interface{}) (json.RawMessage, error) {
			req, ok := obj.(testMethod2RequestServiceInterface)
			if !ok {
				return nil, fmt.Errorf("couldn't assert request as testMethod2RequestServiceInterface, got %T", obj)
			}
			b, err := ffjson.Marshal(req)
			if err != nil {
				return nil, fmt.Errorf("couldn't marshal request %T: %s", obj, err)
