...
func MakeHandlerRESTServiceInterface(s service.Interface, opts ...ServiceInterfaceServerOption) (fasthttp.RequestHandler, error) {
	...
	r.To(fasthttp.MethodGet, "/users/<id:[0-9]>/<name:[a-z]>/<fname>", fasthttp2.NewServer(
		...
		func(ctx context.Context, r *fasthttp.Request) (interface{}, error) {
			...
			q := r.URI().QueryArgs()
			idInt, err := strconv.Atoi(vars.Param("id"))
			if err != nil {
				return nil, fmt.Errorf("convert error: %w", err)
			}
			req.Id = int(idInt)
			req.Name = vars.Param("name")
			req.Fname = vars.Param("fname")
			priceFloat32, err := strconv.ParseFloat(string(q.Peek("price")), 32)
			if err != nil {
				return nil, fmt.Errorf("convert error: %w", err)
			}
			req.Price = float32(priceFloat32)
			nInt, err := strconv.Atoi(string(r.Header.Peek("x-num-n")))
			if err != nil {
				return nil, fmt.Errorf("convert error: %w", err)
			}
			req.N = int(nInt)
			bInt, err := strconv.Atoi(string(r.Header.Peek("x-num-b")))
			if err != nil {
				return nil, fmt.Errorf("convert error: %w", err)
			}
			req.B = int(bInt)
			cInt, err := strconv.Atoi(string(q.Peek("c")))
			if err != nil {
				return nil, fmt.Errorf("convert error: %w", err)
			}
			req.C = int(cInt)
	...
	r.To(http.MethodPut, "/<ns>/auth/<utype>/<user>/<restype>/<resource>/<permission>", fasthttp2.NewServer(
		...
		func(ctx context.Context, r *fasthttp.Request) (interface{}, error) {
			var req testMethod2RequestServiceInterface
			err := ffjson.Unmarshal(r.Body(), &req)
			if err != nil && err != io.EOF {
				return nil, fmt.Errorf("couldn't unmarshal body to testMethod2RequestServiceInterface: %s", err)
			}
			vars, ok := ctx.Value(fasthttp2.ContextKeyRouter).(*routing.Context)
			if !ok {
				return nil, fmt.Errorf("couldn't assert fasthttp2.ContextKeyRouter to *routing.Context")
