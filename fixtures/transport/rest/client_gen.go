...
func ServiceInterfaceTestMethod2ClientOptions(opt ...fasthttp.ClientOption) (_ ServiceInterfaceClientOption) {
	return func(c *clientServiceInterface) { c.testMethod2ClientOption = opt }
}

func ServiceInterfaceTestMethod2ClientEndpointMiddlewares(opt ...endpoint.Middleware) (_ ServiceInterfaceClientOption) {
	return func(c *clientServiceInterface) { c.testMethod2EndpointMiddleware = opt }
}
...
func (c *clientServiceInterface) Get(ctx context.Context, id int, name string, fname string, price float32, n int, b int, c int) (_ user.User, _ error) {
	resp, err := c.getEndpoint(ctx, getRequestServiceInterface{Id: id, Name: name, Fname: fname, Price: price, N: n, B: b, C: c})
	if err != nil {
...
func (c *clientServiceInterface) TestMethod2(ctx context.Context, ns string, utype string, user string, restype string, resource string, permission string) (_ error) {
	_, err := c.testMethod2Endpoint(ctx, testMethod2RequestServiceInterface{Ns: ns, Utype: utype, User: user, Restype: restype, Resource: resource, Permission: permission})
	if err != nil {
		return err
	}
	return nil
...
func NewClientRESTServiceInterface(tgt string, opts ...ServiceInterfaceClientOption) (service.Interface, error) {
	...
	c.testMethod2Endpoint = fasthttp.NewClient(
		...
		u,
		func(_ context.Context, r *fasthttp2.Request, request interface{}) error {
			req, ok := request.(testMethod2RequestServiceInterface)
			if !ok {
				return fmt.Errorf("couldn't assert request as testMethod2RequestServiceInterface, got %T", request)
			}
