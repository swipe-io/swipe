...
func LoadConfig() (cfg *Config, errs []error) {
	...
	nameTmp, ok := os.LookupEnv("NAME")
	if ok {
		cfg.Name = nameTmp
	}
	maxPriceTmp, ok := os.LookupEnv("MAX_PRICE")
	if ok {
		maxpriceInt, err := strconv.Atoi(maxPriceTmp)
		if err != nil {
	...
	dBConnTmp, ok := os.LookupEnv("DB2_CONN")
	if ok {
		cfg.DB.Conn = dBConnTmp
	}
	dBFooNameTmp, ok := os.LookupEnv("DB2_FOO_NAME")
	if ok {
		cfg.DB.Foo.Name = dBFooNameTmp
	}
	uRLsTmp, ok := os.LookupEnv("URLS")
	if ok {
		partsurlsInt := strings.Split(uRLsTmp, ",")
